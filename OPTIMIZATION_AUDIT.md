...
Date: 2026-05-19
Scope: full backend (`backend/app/`) and mobile (`mobile/`) trees. No changes have been applied.
...
### 1.7 Playwright opens a fresh browser context per scrape
...
### 2.2 Two `compute_phash` implementations with different semantics
...
- **Note**: This whole file is dead per 2.4 anyway. Just delete it.
...
### 7.1 Playwright is heavy and may be unnecessary for PriceCharting
...
### 7.2 `react-native-chart-kit` ships ~30KB+ with SVG runtime
...
### 7.3 `axios` + native XHR both shipped
...
- See 6.x. If `tasks/scrape_tasks.py` is removed, `celery` can drop from `requirements.txt`.
...
- This audit was read-only. No files were modified.
- Findings cite line numbers based on current `HEAD` (commit `bc1f387`).
- Numbers/timings are estimates from code inspection; actual measurement is recommended before tackling Tier 1.2 (true streaming) since the latency benefit depends on how skewed crop-search times actually are.
- Several Tier 1 items are quick wins (10 minutes each) that compound noticeably.
